...
function validateSettlementParticipants(fromUserId, toUserId, memberIds) {
  if (!memberIds.includes(fromUserId) || !memberIds.includes(toUserId)) {
    throw new Error('Settlement participants must be members of this Expense Set');
  }
}

function ensureSettlementActorCanRecord(actorUserId, fromUserId, toUserId) {
  if (actorUserId !== fromUserId && actorUserId !== toUserId) {
    throw new Error('Only settlement participants can record this settlement');
  }
}
